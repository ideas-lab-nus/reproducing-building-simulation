...
Here, we use the reference large office building building 
...
repeat {
    ...
    fit_offspring <- evaluate_fitness(control, pop, 0)

    # save fitness in population
    for (i in seq_along(pop)) attr(pop[[i]], "fitness") <- fit[, i]

    cat("  * Prepare next generation\n")
    sel <- ecr::replaceMuCommaLambda(control, pop, offspring, fit, fit_offspring)
    ...
    if (length(stop_obj) > 0L) {
        ...
        cli::cat_line(sprintf("  < %s: %s >", stop_obj$name, stop_obj$message))
        break
    }
}
```
